...
def participant_temoin(autre_participant):
    return autre_participant
...
class TestAdmin:
    ...
    def test_admin_init(self, admin_instance):
        ...
        assert admin_instance.role is True
...
@patch("admin.Evenement")
    def test_creer_evenement(self, MockEvenement, admin_instance):
        ...
        evenement_data = {
            "id_event": 1,
            "date_evenement": date(2025, 10, 20),
            "titre": "fête de fin de projet info",
            "description_evenement": "c'est fini",
            "lieu": "foyer",
            "capacite_max": 4,
            "tarif": 3.00
        }
        ...
        evenement = admin_instance.creer_evenement(
            id_event=evenement_data["id_event"],
            date_evenement=evenement_data["date_evenement"],
            titre=evenement_data["titre"],
            description=evenement_data["description_evenement"],
            lieu=evenement_data["lieu"]
            capacite_max=evenement_data["capacite_max"],
            tarif=evenement_data["tarif"]
        )
    ...
    def test_supp_participant_echec(self, admin_instance, participant_instance, participant_temoin):
        ...
        mock_evenement = Mock(spec=Evenement)
        mock_evenement.desinscrire.return_value = False # simulation de l'échec

        resultat = admin_instance.supp_participant(mock_evenement, participant_instance)

        assert resultat is False # vérification du retour d'échec

        # vérification de l'appel de la méthode même en cas d'échec
        mock_evenement.desinscrire.assert_called_once_with(participant_instance)
